<template>
  ...
  <div class="mw6 center ph3-ns gray lh-copy f3 tc">
      Saved Times
  </div>
        ...
        <div class="fl w-80-ns pa2">
          <ul>
            <li v-for="(name, index) in savedTimes" :key="index">
              ...
              {{ recipe(name).development }}min at {{ recipe(name).temperature }}º in {{ recipe(name).developer }} &nbsp; &nbsp;
              <a class="black-30 hover-gold" href="#" @click.prevent="remove(index, name)">delete</a> &nbsp;
              <a class="black-30 hover-gold" href="#" @click.prevent="start(name)">start</a> &nbsp;
            </li>
            <li>
              <span class="lh-copy f4 normal"><router-link class="black-30 hover-gold" to="/">home</router-link></span>
            </li>
          </ul>
        </div>
      </div>
